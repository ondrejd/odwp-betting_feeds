...
if( ! class_exists( 'BF_Screen_Prototype' ) ):
...
abstract class BF_Screen_Prototype {
    ...
    /**
     * <p>Array with screen options. Don't forget that you can use screen options only when {@see BF_Screen_Prototype::$enable_screen_options} is set on <code>TRUE</code>. You can define them like this:</p>
     * <pre>
     ...
     * <ul>
     *   <li>{@see BF_Screen_Prototype::get_screen_options()}</li>
     *   <li>{@see BF_Screen_Prototype::save_screen_options()}</li>
     *   <li>{@see BF_Screen_Prototype::screen_options()}</li>
     * </ul>
     *
     ...
     * @return array
     * @since 1.0.0
     * @todo Make this automatic without need of writing own code - just use {@see BF_Screen_Prototype::$options}.
     */
     ...
     * for `load-{$hookname}` hook (see {@see BF_Screen_Prototype::admin_menu} 
     * for more details).
     * @return void
     * @since 1.0.0
     */
    ...
    /**
     * <p>Renders screen options form. Handler for `screen_layout_columns` filter (see {@see BF_Screen_Prototype::screen_load}).</p>
     * @return void
     * @since 1.0.0
     * @todo It should be rendered automatically by using {@see BF_Screen_Prototype::$options}.
    ...
    public function screen_options() {
        ...
        ob_start();
        $template = str_replace( BF_SLUG . '-', '', "screen-{$this->slug}_options.phtml" );
        include( BF_PATH . "partials/{$template}" );
        $output = ob_get_clean();
         ...
         */
        $output = apply_filters( BF_SLUG . "_{$this->slug}_screen_options_form", $output );
        echo $output;
    }

    /**
     * <p>Save screen options. Action for `admin_init` hook (see {@see BF_Screen_Prototype::init} for more details). Here is an example code how to save a screen option:</p>
     * <pre>
     * $user = get_current_user_id();
     *
     * if(
     ...
     * @return void
     * @since 1.0.0
     * @todo It should be done automatically by using {@see BF_Screen_Prototype::$options} without need of writing own code.
     */
    ...
    public function render( $args = [] ) {
         ...
         * @param string $output Rendered HTML.
         */
        $output = apply_filters( BF_SLUG . "_{$this->slug}", $output );
        echo $output;
    }
}
